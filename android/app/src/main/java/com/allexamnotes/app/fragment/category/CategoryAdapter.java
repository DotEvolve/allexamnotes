...
public class CategoryAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType==CATEGORY_TYPE){
            ...
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.category_item_layout_2,parent,false);
            return new ItemLayout2ViewHolder(view,onCategoryListener);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (itemType==CATEGORY_TYPE){
            ...
            Category category = (Category) itemsList.get(position);
            ItemLayout2ViewHolder taxanomyViewHolder = (ItemLayout2ViewHolder) holder;
            taxanomyViewHolder.titleView.setText(Jsoup.parse(category.getName()).text());
            if(category.getImage()==null&& Config.AUTO_LOAD_SMART_IMAGES)
                category.setImage("https://source.unsplash.com/200x200/?"+category.getName());
            Glide.with(mContext)
                    .load(category.getImage())
                    .into(taxanomyViewHolder.imageView);
            taxanomyViewHolder.itemView.setOnClickListener(view -> {
                Intent intent = new Intent(mContext, ContainerActivity.class);
                intent.putExtra("title",Jsoup.parse(category.getName()).text());
